...
我们大都可以将异相催化过程拆为若干个基元过程，对于一个基元过程而言，以表面反应为例，反应物首先吸附在表面，然后通过扩散过程进入相互位点之间的邻近区域，再后它们相互反应生成产物，最终脱附。而这些基元过程在催化过程中构成循环，在反应首尾表面还原其最原始的构型。这被称为$催化循环（catalytic\  cycle）$

具体而言，对于这样的反应，以及它在表面催化中构成的催化循环：
$$A_{2}+2B \to 2AB$$
$$A_{2}+2^*\to 2A^*$$
$$A^*+B \to AB+*$$
其中第二步就是在电化学中常常出现的$Eley-Rideal \ step$，整个部分被称为$Heyrovsky机制$当然这里的使用仅仅在于保持讨论的简易。根据它们，我们能分别写出反应的总速率表达式：
$$R_{1}=r_{1}-r_{-1}=k_{1}p_{A_{2}}\theta_{*}^2-k_{-1}\theta_{A}^2$$
$$R_{2}=r_{2}-r_{-2}=k_{2}p_{B}\theta_{A}-k_{-2}p_{AB}\theta_{*}$$
具体而言其中$R_{1}$表示了$A_{2}$解离的速率，而$R_{2}$表示了$AB$生成的速率。
而我们想要讨论的是这样的速率到底怎样相应影响覆盖比：
显而易见地有：
